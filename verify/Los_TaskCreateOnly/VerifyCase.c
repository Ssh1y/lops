...
LITE_OS_SEC_TEXT_INIT INT32 main(VOID)
{
    ...
    UINT16 policy0 = LOS_SCHED_RR; // 对应HPF调度策略
    UINT16 policy1 = LOS_SCHED_DEADLINE; // 对应EDF调度策略


    UINT32 taskID0 = createTaskOnly(policy0); // 创建一个任务但不加入就绪队列，作为当前运行任务
    UINT32 taskID1 = createTaskOnly(policy1); // 创建一个任务但不加入就绪队列，作为当前运行任务
}